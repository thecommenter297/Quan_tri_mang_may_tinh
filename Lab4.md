...
*   Cấu trúc OU `HANOI\Technical` đã được tạo từ Lab 3, cùng với các tài khoản người dùng trong OU này (ví dụ: `hungnq`).

---

### **Phần A: Triển khai các Chính sách Giới hạn Người dùng**

**Mục tiêu:** Tạo một Group Policy Object (GPO) để áp đặt các giới hạn chung cho người dùng trong phòng ban Technical, bao gồm: khóa Registry, khóa Task Manager, cấm Command Prompt, và loại bỏ mục RUN khỏi Start Menu.
...
3.  Chuột phải vào OU **Technical**, chọn **Create a GPO in this domain, and Link it here...**.
4.  Đặt tên cho GPO là `Technical_User_Restrictions` và nhấn **OK**.

**2. Cấu hình các chính sách**
1.  Chuột phải vào GPO `Technical_User_Restrictions` vừa tạo và chọn **Edit...**.
...
1.  Để chính sách có hiệu lực ngay lập tức, mở **Command Prompt** hoặc **PowerShell** trên cả máy chủ và máy client, sau đó chạy lệnh:
    ...
    gpupdate /force
    ```
2.  Trên máy trạm `W10-CLIENT01`, đăng nhập bằng tài khoản thuộc OU Technical (ví dụ: `hungnq`).
3.  Kiểm tra:
...
**Mục tiêu:** Thiết lập một màn hình nền chung cho tất cả người dùng trong phòng Technical để đảm bảo tính đồng bộ.
...
1.  Trong **Group Policy Management**, chuột phải vào OU **Technical** và tạo một GPO mới tên là `Technical_Desktop_Wallpaper`.
...
2.  **Phục hồi (Restore):**
    *   Để giả lập sự cố, bạn có thể xóa một GPO (ví dụ: `Technical_User_Restrictions`).
...
1.  **Sao lưu một GPO cụ thể:**
    ```powershell
    Backup-GPO -Name "Technical_User_Restrictions" -Path "C:\GPO_Backups"
    ```
...
3.  **Phục hồi một GPO:**
    ```powershell
    Restore-GPO -Name "Technical_User_Restrictions" -Path "C:\GPO_Backups"
    ```
...
1.  Tạo một GPO mới liên kết tới OU **Technical**, đặt tên là `Technical_AppLocker_Service`.
